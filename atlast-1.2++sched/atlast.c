...
prim P_dolit()			      /* Push instruction stream literal */
{
    ...
    if (atl_trace) {
        ...
        printf("%ld ", (long) *ip); // NOT EMBEDDED
    ...
    if (i != TokNull) {
        if (i == TokWord) {
            ...
            if ((di = lookup(tokbuf)) != NULL) {
                ...
                printf(" '%s' undefined ", tokbuf); // EMBEDDED
            }
        } else {
